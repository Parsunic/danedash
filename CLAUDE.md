...
- **Rollover retires a day by RECORDING it, never by deleting it** (`rollover_done_v1`, a synced map of retired dates). Deleting the past `goals:` key locally does not work: the server keeps its copy, the next pull restores it, and the next launch rolls it again — so a task the user deleted came back on every single reload, forever. Past days are therefore kept as history (the weekly review reads them). Rollover also skips any task already tombstoned in today's key, so a second pass on the same day cannot undo a deletion.
