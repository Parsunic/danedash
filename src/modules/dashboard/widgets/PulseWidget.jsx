...
// Today's Pulse widget — copied from GoalsPulseCard.jsx (original untouched).
// M: habit dots row + count + verdict line (no milestone).
// L: today's full card (dots + milestone + verdict + footer).
// Verdict logic preserved EXACTLY: same model, prompt, cache key. A module-scope
// in-flight record dedupes the Sonnet call so StrictMode double-mounts and
// remounts never double-bill — later mounts attach to the same promise.
...
// Module-scope in-flight verdict fetch: one API call per day, ever, no matter
// how many times the widget mounts. Subsequent mounts reuse the same promise.
let verdictInflight = { date: null, promise: null }

export default function PulseWidget({ size, bp }) {
  ...
  useEffect(() => {
    ...
    if (verdictInflight.date !== today || !verdictInflight.promise) {
    ...
    verdictInflight.promise.then(text => {
      if (!alive) return
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      <div className="dash-pulse-label" style={{ marginBottom: isL ? 12 : 8 }}>Today's Pulse</div>

      <div className="pulse-habits-row" style={isL ? null : { marginBottom: 0 }}>
      ...
      {(verdict || loadingVerdict) && (
        <div className="pulse-verdict" style={isL ? null : { marginTop: 'auto', marginBottom: 0, paddingTop: 8 }}>
          <span className="pulse-verdict-star">✦</span>
          <span
            className={`pulse-verdict-text${loadingVerdict ? ' is-loading' : ''}`}
            style={isL ? null : {
              display: '-webkit-box',
              WebkitLineClamp: 2,
      ...
      {isL && (
        <div className="pulse-footer" style={{ marginTop: 'auto' }}>
