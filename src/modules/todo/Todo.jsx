...
function findRow(fresh, goal, index) {
  if (goal?.id) {
...
function writeFresh(goalKey, goal, index, fn) {
...
// Reorder against the freshly-read list, anchored on the row the item was dropped onto
// rather than a raw index — a task added remotely shifts every index, and a raw index
// would drop the row in the wrong place (or off the end).
function reorderFresh(goalKey, goals, fromIdx, toIdx) {
  const moved = goals[fromIdx]
  const anchor = goals[toIdx]
  const fresh = storeGet(goalKey) || []
